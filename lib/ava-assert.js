...
function validateExpectations(assertion, expectations, numArgs) { // eslint-disable-line complexity
  if (typeof expectations === 'function') {
    ...
    throw new AssertionError({
      assertion,
      message: `The second argument to \`assert.${assertion}()\` must be a function, string, regular expression, expectation object or \`null\``,
    ...
    if (hasOwnProperty(expectations, 'instanceOf') && typeof expectations.instanceOf !== 'function') {
      throw new AssertionError({
        assertion,
        message: `The \`instanceOf\` property of the second argument to \`assert.${assertion}()\` must be a function`,
        values: [formatWithLabel('Called with:', expectations)],
      });
    }
    ...
    if (hasOwnProperty(expectations, 'message') && typeof expectations.message !== 'string' && !(expectations.message instanceof RegExp)) {
      throw new AssertionError({
        assertion,
        message: `The \`message\` property of the second argument to \`assert.${assertion}()\` must be a string or regular expression`,
        values: [formatWithLabel('Called with:', expectations)],
      });
    }
    ...
    if (hasOwnProperty(expectations, 'name') && typeof expectations.name !== 'string') {
      throw new AssertionError({
        assertion,
        message: `The \`name\` property of the second argument to \`assert.${assertion}()\` must be a string`,
        values: [formatWithLabel('Called with:', expectations)],
      });
    }
    ...
    if (hasOwnProperty(expectations, 'code') && typeof expectations.code !== 'string' && typeof expectations.code !== 'number') {
      throw new AssertionError({
        assertion,
        message: `The \`code\` property of the second argument to \`assert.${assertion}()\` must be a string or number`,
        values: [formatWithLabel('Called with:', expectations)],
      });
    }
    ...
    for (const key of Object.keys(expectations)) {
      switch (key) {
        ...
        default:
          throw new AssertionError({
            assertion,
            message: `The second argument to \`assert.${assertion}()\` contains unexpected properties`,
            values: [formatWithLabel('Called with:', expectations)],
          });
      }
    }
