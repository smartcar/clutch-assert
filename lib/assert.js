...
Object.defineProperty(assert, 'AssertionError', {value: default.AssertionError});
...
function test(ok, opts) {
  if (!ok) {
    throw new default.AssertionError(opts);
  }
}
...
assert.notRegex = function(contents, regex, msg) {
  test(!regex.test(contents), create(regex, contents, '!==', msg, assert.notRegex));
};
